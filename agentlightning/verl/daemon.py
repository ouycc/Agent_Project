...
class AgentModeDaemon:
    ...
    def get_train_data_batch(self, max_prompt_length, max_response_length, device):
        ...
        if n_transition == 0:
            # 如果没有有效数据，返回一个空的批次
            print("Warning: No valid training data available. Returning empty batch.")
        ...
        # 添加安全检查，确保attention_mask不为空
        if attention_mask.numel() > 0:
            eos_mask_idx = torch.argmax(position_ids * attention_mask, dim=-1)  # (bsz,)
            token_level_scores[torch.arange(n_transition), eos_mask_idx] = scores
        else:
            print("Warning: Empty attention_mask detected.")
            eos_mask_idx = torch.zeros(n_transition, dtype=torch.long, device=device)
